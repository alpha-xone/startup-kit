...
This skill defaults to structured Markdown. When the user wants a visual report — a channel comparison or a stage-transition map — render it by **delegating**, not by styling it yourself:

- Visual layer → invoke the **`impeccable`** skill (Read mode for a report, Operate if it is a working dashboard).
